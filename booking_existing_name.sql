-- =====================================================================
-- UltraWash — Réservation publique : privilégier le nom déjà enregistré
-- À exécuter dans Supabase Studio > SQL Editor (idempotent)
-- Pré-requis : booking_client_link.sql déjà exécuté.
-- =====================================================================
-- Si le téléphone correspond à un client existant, on garde le "Nom
-- complet" déjà enregistré dans le système (et non celui retapé dans le
-- formulaire public). Si le numéro est inconnu, on crée la fiche avec le
-- nom saisi. La réponse renvoie aussi client_nom (nom retenu).
-- =====================================================================
...
begin
  ...
  -- Client : reconnaître par téléphone (et privilégier son nom enregistré),
  -- sinon créer la fiche avec le nom saisi.
...
-- Vérification
-- ---------------------------------------------------------------------
-- select public.public_create_booking('Alpha','779912345',null,'Voiture', current_date+1,'10:00',null,null);
-- select public.public_create_booking('Beta', '779912345',null,'Voiture', current_date+1,'11:00',null,null);
--   -> la 2e doit renvoyer client_nom = 'Alpha' (nom déjà enregistré conservé)
